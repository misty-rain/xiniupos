...
import com.xiniu.pos.support.file.FileDownloaderHttpHelper;
...
public class ImageTool {
		...
		if (cutWidth > 0 && cutHeight > 0) {
			...
			try {
				BitmapRegionDecoder decoder = BitmapRegionDecoder.newInstance(
						filePath, false);
				if (decoder != null) {
					Bitmap bitmap = decoder.decodeRegion(new Rect(startX, 0,
							startX + cutWidth, cutHeight), null);
					...
					if (bitmap != null) {
						Bitmap roundedCornerBitmap = ImageEdit
	...
	public static Bitmap getRoundedCornerPic(String url, int reqWidth,
		...
		if (size[0] > 0 && size[1] > 0) {
			Bitmap scaledBitmap = Bitmap.createScaledBitmap(bitmap, size[0],
					size[1], true);
	...
	public static Bitmap getMiddlePictureInBrowserMSGActivity(String url,
		...
		if (file.exists()) {
			DisplayMetrics displayMetrics = GlobalContext.getInstance()
					.getDisplayMetrics();
			return decodeBitmapFromSDCard(filePath, displayMetrics.widthPixels,
					900);
		}
		return null;
		// } catch (OutOfMemoryError ignored) {
	...
	public static String compressPic(Context context, String picPath) {
		...
		/*
		 * switch (SettingUtility.getUploadQuality()) { case 1: return picPath;
		 * case 2: options.inSampleSize = 2; break; case 3: options.inSampleSize
		 * = 4; break; case 4: options.inSampleSize = 2;
		 * 
		 * if (Utility.isWifi(context)) { return picPath; } break;
		 */
